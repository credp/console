...
module framebuffer_agg23_burst_cache #(
    // Number of SDRAM data cycles inserted before p0_q reaches this cache.
    // The direct cache interface used by its unit test has no extra stage.
    ...
    // If an integration registers DQ before this cache, ask the controller to
    // stop that many words early. Its existing final output cycles then drain
    // through the integration register without losing the end of the page.

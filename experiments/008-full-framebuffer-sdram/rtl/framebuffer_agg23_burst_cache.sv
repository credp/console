...
module framebuffer_agg23_burst_cache (
    ...
    typedef enum logic [2:0] { IDLE, FETCH_WAIT, FETCH_PULSE, FETCH,
                               FETCH_DONE, SERVE_PRIME, SERVE, COMPLETE } state_t;
    state_t state;
    (* ramstyle = "M10K" *) logic [15:0] page0 [0:1023];
    (* ramstyle = "M10K" *) logic [15:0] page1 [0:1023];
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            if (state == FETCH && p0_data_available) begin
                if (fetch_buffer)
                    page1[fetch_index] <= p0_q;
                else
                    page0[fetch_index] <= p0_q;
            ...
            case (state)
                ...
                SERVE_PRIME: begin
                    read_data <= serve_buffer ? page1[serve_index] : page0[serve_index];
                    read_valid_q <= 1'b1;
                    state <= SERVE;
                end
                SERVE: if (read_valid_q && read_ready) begin
                    if (serve_index + 1'b1 == request_offset_q + words_q) begin
                        ...
                        read_data <= serve_buffer ? page1[serve_index + 1'b1] :
                                                   page0[serve_index + 1'b1];
                    end
                end

...
// Five fixed hexadecimal fields in the top-left diagnostic strip. Fields are
// writer, reader, errors, scanout underflows, and readback-probe status.
module framebuffer_debug_overlay(
    ...
    input logic [7:0] probe_status,
    output logic override, output logic [15:0] pixel
);
    logic [15:0] value; logic [3:0] nibble; logic [6:0] segments;
    logic [2:0] field; logic digit; logic [2:0] dx; logic [3:0] dy;
    always_comb begin
        override=0; pixel='0; field=x[6:4]; digit=x[3]; dx=x[2:0]; dy=y[3:0];
        case(field)
          0:value=writer_count;
          1:value=reader_count;
          2:value=error_count;
          3:value=underflow_count;
          default:value={8'h00,probe_status};
        endcase
        ...
        if(active && y<10 && x<80 && ((dy==0&&segments[6])||(dy==9&&segments[3])||(dx==0&&dy<5&&segments[5])||(dx==0&&dy>4&&segments[4])||(dx==5&&dy<5&&segments[1])||(dx==5&&dy>4&&segments[2])||(dy==4&&segments[0]))) begin
          override=1;
          pixel=(field==2 || (field==4 && probe_status!=8'h01)) ? 16'hf800 :
                (field==3) ? 16'hffe0 : (field==4) ? 16'h07e0 : 16'hffff;
        end
    end
endmodule
